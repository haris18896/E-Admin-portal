...
const KioskModal = ({ loading, open, handleModal }) => {
  const [type, setType] = useState('kiosk')
  const dispatch = useDispatch()
  const navigate = useNavigate()

  const KioskModalSchema = Yup.object().shape({
  ...
  const formik = useFormik({
    ...
    validationSchema: KioskModalSchema,
  ...
  return (
    ...
    >
      <ModalHeader
        ...
        tag="div"
      >
        <h5 className="modal-title ethera-dark fw-600">Register Kiosk</h5>
      </ModalHeader>

      <PerfectScrollbar options={{ wheelPropagation: false }}>
        <ModalBody className="flex-grow-1 pb-sm-0 pb-3">
          <Form name="kiosk-board" onSubmit={formik.handleSubmit}>
            <div className="px-2">
              <FormGroupElement
                inputType="text"
                label="Email"
